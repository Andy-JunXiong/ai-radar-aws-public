...
type WorkspaceItem = {
  ...
  career_takeaway?: string;
  career_relevance?: string;
};

function parseCareerTakeaway(item: WorkspaceItem): string {
  const candidates = [
    item.relevance_to_career,
    item.career_takeaway,
...
export default function CareerTakeawaysPage() {
  ...
  const careerRows = useMemo(() => {
    ...
    for (const item of items) {
      ...
      const takeaway = parseCareerTakeaway(item);
    ...
    <AppContainer style={{ paddingTop: "24px" }}>
      <div style={headerPanelStyle}>
        <div>
          ...
          <h1 style={pageTitleStyle}>Career Takeaways</h1>
          <p style={descriptionStyle}>Browse signal takeaways filtered by career relevance.</p>
        </div>
      ...
      {loading ? (
        ...
        >
          Loading career takeaways from workspace history...
        </div>
      ) : careerRows.length === 0 ? (
        <div
          style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          >
            {formatSignalCount(careerRows.length)} with <strong>career relevance</strong>
          </div>
