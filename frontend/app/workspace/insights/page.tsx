...
function parseInsight(item: WorkspaceItem): string {
  const candidates = [
    item.synthesized_insight,
    item.strategic_insight,
    item.insight,
  ];
...
export default function StrategicInsightsPage() {
  ...
  const insightRows = useMemo(() => {
    ...
    for (const item of items) {
      ...
      const takeaway = parseInsight(item);
    ...
    <AppContainer style={{ paddingTop: "24px" }}>
      <div style={headerPanelStyle}>
        <div>
          ...
          <h1 style={pageTitleStyle}>Strategic Insights</h1>
          <p style={descriptionStyle}>Browse synthesized signal insights across saved workspace records.</p>
        </div>
      ...
      {loading ? (
        ...
        >
          Loading strategic insights from workspace history...
        </div>
        ...
        >
          No strategic insights found yet.
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          >
            {formatSignalCount(insightRows.length)} with <strong>strategic insight</strong>
          </div>
