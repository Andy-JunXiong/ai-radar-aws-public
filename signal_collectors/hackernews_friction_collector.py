...
HN_SEARCH_WINDOW_DAYS = int(os.getenv("AI_FRICTION_HN_SEARCH_WINDOW_DAYS", "14"))
HN_PER_QUERY_LIMIT = max(1, min(int(os.getenv("AI_FRICTION_HN_PER_QUERY_LIMIT", "10")), 20))
HN_MIN_POINTS = max(0, int(os.getenv("AI_FRICTION_HN_MIN_POINTS", "10")))
...
def _hn_request(url: str) -> Any:
    req = request.Request(
        url,
        headers={"User-Agent": "AI-Radar-Friction/1.0"},
        method="GET",
    )
...
def _matched_keywords(*values: str) -> list[str]:
    ...
    if not haystack:
        return []
    return [keyword for keyword in HN_FRICTION_KEYWORDS if keyword in haystack]
...
def _normalize_hit(hit: dict[str, Any], search_term: str) -> dict[str, Any] | None:
    ...
    summary = (
        "Hacker News friction discussion detected around AI tooling or agent workflows. "
    ...
    return {
        ...
        "author": author,
        "source": "hackernews",
        "source_type": "hackernews_friction",
        ...
        "source_weight": 0.88 if points >= 100 else 0.78 if points >= 40 else 0.68,
...
def save(signals: list[dict[str, Any]]) -> None:
    ...
    payload = {
        ...
        "source": "hackernews_friction_collector",
        "count": len(signals),
        "signals": signals,
    }
    OUTPUT_FILE.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"[hn_friction] saved {len(signals)} signals to {OUTPUT_FILE}")


if __name__ == "__main__":
    collected = collect_hackernews_friction_signals()
    save(collected)
    print(f"[hn_friction] done, total signals: {len(collected)}")
