...
REFLECTIONS_FILE = BASE_DIR / "output" / "reflections.json"
PERSONAL_CONTEXT_FILE = BASE_DIR / "app" / "context" / "personal_context.json"
MANUAL_UPLOADS_DIR = BASE_DIR / "output" / "manual_uploads"
MANUAL_SESSIONS_DIR = BASE_DIR / "output" / "manual_sessions"
...
def load_manual_sessions() -> list[dict]:
    ensure_manual_session_dirs()
    sessions = []
    for path in sorted(MANUAL_SESSIONS_DIR.glob("*.json"), reverse=True):
        try:
...
def render_uploaded_file_list(title: str, files: list):
    ...
    for idx, f in enumerate(files, start=1):
        ...
        st.write(f"{idx}. {filename} ({size_kb} KB)")
...
def render_gallery(title: str, images: list, state_prefix: str, is_uploaded: bool = True):
    ...
    for idx, img in enumerate(images):
        with thumb_cols[idx % 3]:
            if is_uploaded:
                st.image(img, caption=getattr(img, "name", f"Image {idx + 1}"), use_container_width=True)
            else:
                st.image(str(img), caption=Path(str(img)).name, use_container_width=True)
    ...
    st.image(selected, caption=getattr(selected, "name", f"Image {selected_num}"), use_container_width=True)
...
st.caption("Signal → Insight → Reflection → Multi-Model Compare")
...
with manual_left:
    ...
    if st.button(analyze_button_label, use_container_width=True, disabled=analyze_btn_disabled):
        if preview_images and not preview_docs:
            ...
            # Preserve the existing file-analysis result.
            ...
            # Convert uploaded documents into signals.
    ...
    if current_manual_result:
        if current_manual_result.get("error"):
            ...
            if st.button("Save This Manual Vision Session", use_container_width=True):
                ...
                session_id = build_manual_session_id(session_title_value)
                saved_paths = save_uploaded_files_to_session(session_id, preview_files)
                ...
                session_payload = {
                    ...
                    "images": [p for p in saved_paths if str(p).lower().endswith((".png", ".jpg", ".jpeg", ".webp"))],
                    "files": saved_paths,
                    ...
                    "project_takeaways": english_payload.get("project_takeaways", []),
                    "career_takeaways": english_payload.get("career_takeaways", []),
                ...
                save_manual_session(session_payload)
...
if manual_sessions:
    ...
    for sess in manual_sessions:
        session_labels.append(
            f"{safe_text(sess.get('created_at', ''))} | {safe_text(sess.get('session_title', 'Manual vision session'))} | {sess.get('file_count', sess.get('image_count', 0))} files"
        )
    ...
    selected_manual_session = manual_sessions[selected_session_idx]

    ms1, ms2, ms3 = st.columns(3)
    ms1.metric("Saved sessions", len(manual_sessions))
    ms2.metric("Files", selected_manual_session.get("file_count", selected_manual_session.get("image_count", 0)))
    ...
    with s2:
        render_scroll_text_box(
            ...
            format_takeaways_list(selected_manual_session.get("project_takeaways", [])),
            height=220,
        )
        render_scroll_text_box(
            "Saved career takeaways (English)",
            format_takeaways_list(selected_manual_session.get("career_takeaways", [])),
            height=220,
        )
else:
